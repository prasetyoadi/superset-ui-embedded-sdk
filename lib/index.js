...
async function embedDashboard(_ref) {
  ...
  function calculateConfig() {
    ...
    if (dashboardUiConfig) {
      ...
      if (dashboardUiConfig.hideDownload) {
        configNumber += 16;
      }
    }
  ...
  async function mountIframe() {
    return new Promise(resolve => {
      ...
      if (!!extraUrlParams) extraUrlParams = "&" + extraUrlParams; // set up the iframe's sandbox configuration
      ...
      iframe.src = `${supersetDomain}/embedded/${id}${dashboardConfig}${filterConfigUrlParams}${extraUrlParams}`; //@ts-ignore
